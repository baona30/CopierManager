...
function EmployeeDetails() {
  ...
  const submitEmployeeRecord = async (e) => {
    ...
    try {
      ...
      if (res.status === 200) {
        alert("Data Inserted");
        setUser({fname: "",lname: "",email: "",password: "",role: ""});
      }
      fetchDatas();
    } catch (err) {
      console.log(err);
                ...
                {datashow.map((item, index) => (
                  <tr>
                    ...
                    <td>
                      ...
                      <span
                        ...
                        onClick={() => {
                          const confirmBox = window.confirm(
                            "Do you really want to delete " + item.employeeFName +"id:" + item.employeeID
                          );
                          if (confirmBox === true) {
                            deleteRecord(item.employeeID);
                          }
                        }}
                      >
                        <i
                      ...
                      <NavLink
                        className="mr-2"
                        to={`/employee/editID/${item.employeeID}`}
                      >
                        <i
                          class="text-warning"
                          aria-hidden="true"
